# -*- coding: utf-8 -*-
"""
Python source file parsing and analysis.
...
copyright:
    "Copyright 2019 High Integrity Artificial Intelligence Systems"
...
requirements:
    "This design document may contain any number of requirements
    items embedded in comment blocks. Each requirements item
    that is present shall have one formal requirement statement
    with one modal verb. The modal verb shall be interpreted
    in accordance with iso29148:2011

    SHALL indicates a mandatory binding provision, the fulfillment
    of which must be proven through an appropriate test.

    WILL indicates a statement of fact; futurity; a declaration
    of purpose; or to establish a context or a limitation of use.
    These statements are not manadatory and not binding.

    SHOULD indicates a preference; a goal or an advantageous
    quality. These statements are not mandatory and not binding.

    MAY is used to grant permission or to make a suggestion.
    These statements are not mandatory and not binding."
...
"""
...
MetaNode     = collections.namedtuple('MetaNode',
                ['lo', 'hi',  'ctx_lo', 'ctx_hi', 'ctx_typ', 'path', 'node'])
...
# =============================================================================
class CommentType(enum.Enum):                           # pylint: disable=R0903
    """
    Enum for the types of comments that occur in a Python source file.
...
def iter_embedded_data(module_name, root, file):
    ...
    for embed in _gen_embedded_data_in_file(file):
        ...
        is_comment = embed.meta.ctx_typ == CommentType.COMMENT
        ...
        if next_path and is_comment and embed_hi + 1 == next_path[-1].lineno:
            ...
            yield Embedded(dat  = embed.dat,
                           meta = MetaNode(lo      = embed.meta.lo,
                                           hi      = embed.meta.hi,
                                           ctx_lo  = embed.meta.ctx_lo,
                                           ctx_hi  = embed.meta.ctx_hi,
...
# -----------------------------------------------------------------------------
def gen_functions_and_methods(module_name, source_text, root_node):
    """
    Yield functions and methods in the specified file.

    This is used by pycomplexity to determine the
    design elements for which complexity metrics
    ...
    for path in gen_ast_paths_depth_first(root_node, module_name):
        ...
        # The function text is needed to calculate
        # raw Radon metrics.
        #
...
def _gen_embedded_data_in_file(file):
    ...
    """
    for embed in _gen_embedded_data(_gen_comment_blocks_in_file(file)):
        yield embed


# -----------------------------------------------------------------------------
def _gen_comment_blocks_in_file(file):
    """
    Yield comment blocks in the specified file.
    ...
    """
    for block in _merge_comment_blocks(
...
def _gen_embedded_data(gen_blocks):
    ...
    for comment in gen_blocks:
        for embedded in _gen_embedded_data_in_comment(comment):
            yield embedded


# -----------------------------------------------------------------------------
def _gen_embedded_data_in_comment(comment):
    """
    Yield Embedded named tuples populated with YAML from a supplied text block.
    ...
    for (iline, line) in enumerate(iter_lines, start = ctxmeta.lo):
        ...
        # State transition: inside to outside
            ...
            if yaml_lines:
                ...
                #       WHAT ABOUT USING SAFE_LOAD_ALL ???
...
def _merge_comment_blocks(gen_comm):
    ...
    for (this_com, next_com) in _iter_lookahead(gen_comm):
        ...
        is_continued = (     next_com is not None
                         and this_com.meta.typ == CommentType.COMMENT
